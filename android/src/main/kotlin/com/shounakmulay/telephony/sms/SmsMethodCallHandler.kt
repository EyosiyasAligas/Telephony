...
class SmsMethodCallHandler(
            ...
            ActionType.BACKGROUND -> {
                if (call.hasArgument(SETUP_HANDLE)
                    ...
                    if (setupHandle == null || backgroundHandle == null) {
                        result.error(
                            ILLEGAL_ARGUMENT,
                            "Setup handle or background handle missing",
                            null
                        )
                        return
                    }
    ...
    private fun handleGetSmsActions(smsAction: SmsAction) {
        if (projection == null) {
            projection =
                if (smsAction == SmsAction.GET_CONVERSATIONS) DEFAULT_CONVERSATION_PROJECTION else DEFAULT_SMS_PROJECTION
        }
        ...
        val messages =
            smsController.getMessages(contentUri, projection!!, selection, selectionArgs, sortOrder)
    ...
    private fun handleSendSmsActions(smsAction: SmsAction) {
        ...
        when (smsAction) {
            SmsAction.SEND_SMS -> smsController.sendSms(address, messageBody, listenStatus, subId, messageId)
            SmsAction.SEND_MULTIPART_SMS -> smsController.sendMultipartSms(
                address,
                messageBody,
                listenStatus,
                subId,
            ...
            val value: Any = when (smsAction) {
                ...
                SmsAction.GET_SIGNAL_STRENGTH -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        ...
                        result.error(
                            ...
                            "getServiceState() can only be called on Android Q and above",
                            null
                        )
                    }
                }
                ...
                SmsAction.GET_SERVICE_STATE -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        ...
                        result.error(
                            "INCORRECT_SDK_VERSION",
                            "getServiceState() can only be called on Android O and above",
                            null
                        )
                    }
                }
                ...
                SmsAction.GET_SUBSCRIPTION_LIST -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                        ...
                        result.error(
                            "INCORRECT_SDK_VERSION",
                            "getSubscriptionList() can only be called on Android Lollipop MR1 (API 22) and above",
                            null
                        )
                    }
                }
    ...
    private fun handleMethod(smsAction: SmsAction, requestCode: Int) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M || checkOrRequestPermission(
                smsAction,
                requestCode
            )
        ) {
        ...
        when (smsAction) {
            ...
            SmsAction.REQUEST_PHONE_AND_SMS_PERMISSIONS -> {
                val permissions = listOf(
                    permissionsController.getSmsPermissions(),
                    permissionsController.getPhonePermissions()
                ).flatten()
                return checkOrRequestPermission(permissions, requestCode)
            }
    ...
    override fun onReceive(ctx: Context?, intent: Intent?) {
        if (intent != null) {
            ...
            Log.d("SmsMethodCallHandler", "Received broadcast with action: ${intent.action}, messageId: $messageId")
            when (intent.action) {
                Constants.ACTION_SMS_SENT -> {
                    val status = when (getResultCode()) {
                        Activity.RESULT_OK -> "sent"
                        SmsManager.RESULT_ERROR_GENERIC_FAILURE -> "error_no_balance"
                        SmsManager.RESULT_ERROR_NO_SERVICE -> "error_no_signal"
                        SmsManager.RESULT_ERROR_RADIO_OFF -> "error_airplane_mode"
                        else -> "error_unknown"
                    }

                    // Return both the status AND the messageId to Flutter
                    val resultData = hashMapOf("messageId" to messageId, "status" to status)
                    foregroundChannel.invokeMethod(SMS_SENT, resultData)
                }

                Constants.ACTION_SMS_DELIVERED -> {
                    if (getResultCode() == Activity.RESULT_OK) {
                        val resultData =
                            hashMapOf("messageId" to messageId, "status" to "delivered")
                        foregroundChannel.invokeMethod(SMS_DELIVERED, resultData)
                    }
                }
            }
        }
